from .symbolic import Symbol, Equation
from .ecc import EllipticCurve
from .qap import QAP

class ConstraintSystem():

    def __init__(self, inputs: list, output: str, curve='BN128'):
        self.vars = {}
        self.constraints = []
        self.inputs = inputs
        self.output = output
    ...
    def __get_witness_vector(self):
        ...
        private_input = [v for v in self.vars if v in self.inputs and v not in self.public]
        intermediate_vars = [v for v in self.vars if v not in self.inputs and v != self.output]
    ...
    def evaluate(self, input_values: dict, output_value: int) -> bool:
        ...
        for input in self.inputs:
            self.vars[input] = input_values.get(input, 0) % self.p
        ...
        for constraint in self.constraints:
            ...
            if left.op == 'VAR':
                self.vars[left.name] = eval(str(right), None, self.vars) % self.p
            else:
                evaluated_left = eval(str(left), None, self.vars) % self.p
                evaluated_right = eval(str(right), None, self.vars) % self.p

                return evaluated_left == evaluated_right

        return output_value == self.vars[self.output]
    ...
    def solve(self, input_values: dict, output_value: int) -> list:
        ...
        Returns:
            witness: Witness vector of all variables
        """
        witness = self.__get_witness_vector()
