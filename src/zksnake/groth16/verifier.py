...
class Verifier:
    ...
    def verify(self, proof: Proof, public_witness: list) -> bool:
        ...
        """
        assert len(self.key.kgamma1) == len(
            public_witness
        ), "Length of kgamma1 and public_witness must be equal"

        gamma_witness = [
            point * scalar for point, scalar in zip(self.key.kgamma1, public_witness)
        ]
        ...
        return self.E.pairing(proof.B, proof.A) == self.E.pairing(
            self.key.beta2, self.key.alpha1
        ) * self.E.pairing(self.key.gamma2, sum_gamma_witness) * self.E.pairing(
            self.key.delta2, proof.C
        )
