...
from ..polynomial import (
    ...
    fft,
    get_nth_root_of_unity, ifft,
    mul_over_evaluation_domain, mul_over_fft,
...
class Plonk():
    ...
    def setup(self, g1_tau = None, g2_tau = None):
        ...
        pk = ProvingKey(
            n, self.G1_tau,
            selector_poly, selector_evaluations,
            sigma_permutation_poly,
            identity_permutation_poly,
            tau_selector, tau_permutation, lagrange_evals,
            self.E.name
        )
    ...
    def prove(self, public_witness: dict, private_witness: list):
        ...
        with Timer('round1 B'):
            ...
            g_eval = add_over_evaluation_domain(n*4, [
                a_ql, b_qr, c_qo, ab_qm, selector_eval['C'], pi_eval], self.order)
        ...
        with Timer('round2 A'):
            ...
            eval_denom_poly = mul_over_evaluation_domain(n*4, ab_sigma, c_sigma, self.order)
        ...
        with Timer('round3 A'):
            ...
            Z_omega = PolynomialRing([coeff * self._roots[i % n] % self.order for i, coeff in enumerate(Z.coeffs())], self.order)
            ...
            z_1_l1_eval = mul_over_evaluation_domain(n*4, z_1_eval, self.proving_key.lagrange_evals, self.order)
        ...
        with Timer('round5'):
            ...
            R = (
                ...
                ((T_lo + pow(zeta, n, self.order) * T_mid + T_hi * pow(zeta, n*2, self.order)) * Zh(zeta))
            )
            ...
            divisor_W_zeta_omega = PolynomialRing([-(zeta*self._roots[1]) % self.order, 1], self.order)
            ...
            tau_W_zeta_omega = self.E.multiexp(self.proving_key.tau_g1, W_zeta_omega.coeffs())
    ...
    def verify(self, proof: Proof, public_input: dict):
        ...
        beta, gamma, alpha, zeta, v, u = self.__recompute_challenges(proof, public_input)
        ...
        lhs = self.E.pairing(
            proof.tau_W_zeta + u * proof.tau_W_zeta_omega,
            self.verifying_key.tau_g2
        )

        rhs = self.E.pairing(
            zeta * proof.tau_W_zeta + (u*zeta*omega)*proof.tau_W_zeta_omega + tau_F - tau_E,
            self.E.G2()
        )
